...
function snapshotStockItem(product, inventory, qty, note = '') {
  ...
  const paidCost = cost * qty
  const stockProductId = `stock:${product.id}`
  const stockProductName = `${product.name}【現貨】`
  return {
    id:stockProductId,
    ...
    supplier_paid_amount:paidCost,
    supplier_payment_status:'paid',
    supplier_payment_refs:['stock_inventory'],
    qty,
    subtotal:price * qty,
    cost_subtotal:paidCost,
