...
export const HelperAPI = {
  ...
  async createEntries(entries=[]){
    ...
    for(let i=0;i<entries.length;i+=400){
      const batch=writeBatch(db); const createdAt=now()
      entries.slice(i,i+400).forEach(data=>{ const ref=doc(collection(db,'helper_entries')); batch.set(ref,{...data,status:'pending',created_at:createdAt,updated_at:createdAt}); created+=1 })
      await batch.commit()
    }
    return created
  },
