...
const normalize = d => {
  ...
  ;['created_at','updated_at','converted_at'].forEach(k => { if (x[k]) x[k] = toISO(x[k]) })
  return x
}
...
async function hydrateOrderItems(lines = []) {
  const ids = [...new Set(lines.map(x => x.product_id).filter(Boolean))]
  ...
  return lines.map(line => {
    ...
    if (!product || product.active === false) throw new Error(`商品「${line.product_name || ''}」不存在或已封存`)
