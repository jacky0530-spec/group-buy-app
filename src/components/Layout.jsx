...
import { ShoppingBag, Users, ShoppingCart, BarChart2, Home, Menu, X, UserCog, ClipboardList } from 'lucide-react'
    ...
    <div className="app-layout">
        ...
        <nav className="sidebar-nav">
          {NAV.map(({ to, icon:Icon, label, end }) => (
            <NavLink key={to} to={to} end={end} className={({ isActive }) => `sidebar-nav-link ${isActive ? 'active' : ''}`}>
              {({ isActive }) => <><Icon size={17}/><span>{label}</span>{isActive && <span className="sidebar-active-dot"/>}</>}
            </NavLink>
          ))}
        </nav>
      ...
      <div id="main-content" className="app-shell-content print-content">
        <header className="mobile-header no-print">
          <button type="button" className="mobile-menu-button" onClick={() => setSideOpen(p => !p)} aria-label={sideOpen ? '關閉選單' : '開啟選單'} aria-expanded={sideOpen}>
            {sideOpen ? <X size={22}/> : <Menu size={22}/>} 
          </button>
          <div className="mobile-brand-logo"><ShoppingBag size={16} color="white"/></div>
          <div className="mobile-brand-text"><strong>團購百貨</strong><span>管理系統 · {APP_VERSION}</span></div>
        </header>
