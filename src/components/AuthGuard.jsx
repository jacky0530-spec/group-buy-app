...
export function AuthProvider({ children }) {
  ...
  return (
    <AuthContext.Provider value={{ user, allowed, logout }}>
      {children}
...
function NoPermission({ user }) {
  return (
    <div style={{
      ...
      padding: 20, fontFamily: "'Noto Sans TC', sans-serif",
    }}>
...
// ── AuthGuard（包住所有需要登入的路由）────────────────────────
export function AuthGuard() {
  const { user, allowed } = useAuth()

  // 1. Firebase 還在初始化，顯示載入畫面
    ...
    <div style={{ position: 'relative' }}>
      <button onClick={() => setOpen(p => !p)}
        ...
        <div style={{ flex: 1, textAlign: 'left', minWidth: 0 }}>
          <div style={{ color: '#fff', fontSize: 13, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{name}</div>
          <div style={{ color: 'rgba(255,255,255,.35)', fontSize: 10, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{email}</div>
        </div>
      ...
      {open && (
        <>
          ...
          <div style={{ position: 'absolute', bottom: 'calc(100% + 8px)', left: 0, right: 0, background: '#1e293b', border: '1px solid rgba(255,255,255,.12)', borderRadius: 12, overflow: 'hidden', boxShadow: '0 8px 32px rgba(0,0,0,.4)', zIndex: 99 }}>
