...
export default function HelperStockPanel(){
  ...
  const load=useCallback(async()=>{
    try{setStock(await InventoryAPI.listStock())}
  ...
  return <div>
    <div style={{marginBottom:14}}><strong style={{fontSize:18}}>現貨查詢／開單</strong><div style={{fontSize:12,color:'var(--text-muted)',marginTop:3}}>現貨直接讀 Neon；客戶改為輸入時 SQL 搜尋，不再下載全部客戶。</div></div>
    <div className="card" style={{marginBottom:12}}><div className="card-body"><label style={{fontSize:12,fontWeight:800}}>1. 選擇客戶</label><input value={customerSearch} onChange={e=>{setCustomerSearch(e.target.value);setCustomer(null)}} placeholder="姓名／末碼／備註" style={{...SEARCH_INPUT_STYLE,marginTop:8}}/>{customerSearch&&!customer&&<div style={{border:'1px solid var(--border)',borderRadius:8,maxHeight:220,overflowY:'auto',marginTop:6}}>{customerSearching?<div style={{padding:'12px 14px',color:'var(--text-muted)'}}>SQL 搜尋中...</div>:custs.length?custs.map(c=><button key={c.id} onClick={()=>{setCustomer(c);setCustomerSearch(c.name)}} style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',border:0,borderBottom:'1px solid var(--border)',background:'#fff'}}><strong>{c.name}</strong> <span style={{fontSize:11,color:'var(--text-muted)'}}>末碼 {getCustomerPhoneLast2(c)||'—'}</span></button>):<div style={{padding:'12px 14px',color:'var(--text-muted)'}}>找不到符合客戶</div>}</div>}</div></div>
    <div className="card" style={{marginBottom:12}}><div className="card-body"><label style={{fontSize:12,fontWeight:800}}>2. 搜尋現貨商品</label><div className="search-input-wrap" style={{marginTop:8,width:'100%',maxWidth:'100%'}}><Search size={18}/><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="輸入商品／規格／供應商搜尋" style={{...SEARCH_INPUT_STYLE,paddingLeft:38}}/></div><div style={{display:'grid',gap:8,marginTop:12}}>{filtered.map(r=><button key={r.id} onClick={()=>setSelected(r)} style={{textAlign:'left',padding:'12px 14px',borderRadius:10,border:`2px solid ${selected?.id===r.id?'#059669':'var(--border)'}`,background:selected?.id===r.id?'#ecfdf5':'#fff'}}><div style={{display:'flex',justifyContent:'space-between',gap:10}}><strong>{r.product_name}</strong><strong style={{color:'#059669'}}>現貨 {r.available_qty}</strong></div><div style={{fontSize:12,color:'#2563eb',fontWeight:800,marginTop:3}}>{stockSpecLabel(r.spec)}</div><div style={{fontSize:11,color:'var(--text-muted)',marginTop:2}}>供應商：{r.supplier||'未設定'}</div></button>)}</div></div></div>
