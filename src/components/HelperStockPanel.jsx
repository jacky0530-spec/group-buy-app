...
export default function HelperStockPanel(){
  ...
  return <div>
    ...
    <div className="card" style={{marginBottom:12}}><div className="card-body"><label style={{fontSize:12,fontWeight:800}}>1. 選擇客戶</label><input value={customerSearch} onChange={e=>{setCustomerSearch(e.target.value);setCustomer(null)}} placeholder="姓名／末碼／備註" style={{...SEARCH_INPUT_STYLE,marginTop:8}}/>{customerSearch&&!customer&&<div style={{border:'1px solid var(--border)',borderRadius:8,maxHeight:220,overflowY:'auto',marginTop:6}}>{custs.map(c=><button key={c.id} onClick={()=>{setCustomer(c);setCustomerSearch(c.name)}} style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',border:0,borderBottom:'1px solid var(--border)',background:'#fff'}}><strong>{c.name}</strong> <span style={{fontSize:11,color:'var(--text-muted)'}}>末碼 {getCustomerPhoneLast2(c)||'—'}</span></button>)}</div>}</div></div>
