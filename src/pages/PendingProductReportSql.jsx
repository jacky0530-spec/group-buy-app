...
export default function PendingProductReportSql(){
  ...
  const queryOrders=useCallback(async()=>{
    ...
    const buyerSqlSearch=(selectedBuyerLookup||buyerSearch).trim()
    if(mode==='buyer'&&!buyerSqlSearch){setOrders([]);setLoading(false);return}
    ...
    try{
      ...
      const rows=await fetchReportOrders({
        ...
        search:mode==='buyer'?buyerSqlSearch:(mode==='product'&&!directProductId?(selectedProduct?.name||''):''),
        includeArchived:shipmentView==='shipped'&&showArchived,
      })
  ...
  const buyerMatchSearch=(selectedBuyerLookup||buyerSearch).trim()
  const buyerCandidates=useMemo(()=>buyerRows.filter(row=>matchesBuyer(row,buyerMatchSearch)),[buyerRows,buyerMatchSearch])
  const visibleBuyerCandidates=selectedBuyerLookup&&buyerCandidateSnapshot.length?buyerCandidateSnapshot:buyerCandidates
  ...
  const viewLabel=shipmentView==='shipped'?'已出貨':arrivalView==='arrived'?'已到貨可先出貨':arrivalView==='missing'?'尚未到貨':'全部待出貨'
  ...
  async function confirmRowArchive(){const row=archiveConfirmRow;if(!row||archivingKey)return;setArchiveConfirmRow(null);await changeRowArchive(row,true)}
  ...
  async function changeSourceQty(source,value){
    ...
    if(!Number.isInteger(qty)){toast('訂購量請輸入整數','warning');return}
    if(qty===Number(source.qty))return
    if(source.is_virtual&&qty===0){
      setCancelVirtualSource(source)
      return
    }
    if(qty<1){
      toast(source.is_virtual?'虛擬訂單可輸入 0 取消品項，或輸入至少 1 件':'正式訂單的訂購量至少為 1','warning')
      return
    }
    try{
      await OrdersAPI.updateItemQty(source.order_id,source.item_index,qty)
      toast(`訂購量已更新為 ${qty} ✓`)
      await refresh()
    }catch(err){toast('修改訂購量失敗：'+err.message,'error')}
  }
  async function confirmCancelVirtualItem(){
    const source=cancelVirtualSource
    if(!source||cancelVirtualBusy)return
    setCancelVirtualBusy(true)
    try{
      const result=await OrdersAPI.cancelVirtualItem(source.order_id,source.item_index)
      setCancelVirtualSource(null)
      if(result?.order_cancelled)toast('✅ 已取消最後一個虛擬品項，整張虛擬訂單已取消')
      else toast(`✅ 已取消虛擬品項：${source.spec||source.product_name||'品項'}`)
      await refresh()
    }catch(err){toast('取消虛擬品項失敗：'+err.message,'error')}
    finally{setCancelVirtualBusy(false)}
  }
  async function markSourcePickedUp(source){
  ...
  return <div className="animate-fade">
    ...
    {cancelVirtualSource&&<ConfirmDialog danger={true} message={`確定取消這個虛擬品項？\n${cancelVirtualSource.product_name||'商品'}｜${cancelVirtualSource.spec||'一般規格'}｜原訂 ${cancelVirtualSource.qty} 件\n\n取消後此品項會從這張虛擬訂單移除；其他規格／品項維持不變。若這是最後一個品項，整張虛擬訂單會改為取消。`} onCancel={()=>{if(!cancelVirtualBusy)setCancelVirtualSource(null)}} onConfirm={confirmCancelVirtualItem}/>}
    <div className="no-print" style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:12,flexWrap:'wrap',marginBottom:20}}><div><h2 style={{fontSize:22,fontWeight:800}}>出貨查詢報表</h2><p style={{color:'var(--text-secondary)',fontSize:13,marginTop:2}}>SQL 篩選版：部分先出貨也可獨立封存；剩餘未到貨／未出貨品項仍保留待出貨</p></div><div style={{display:'flex',gap:8}}><button className="btn btn-ghost" disabled={!canOutput} onClick={exportCurrent}><Download size={14}/>匯出 CSV</button><button className="btn btn-primary" disabled={!canOutput} onClick={()=>window.print()}><Printer size={14}/>列印</button></div></div>
    ...
    <div className="no-print" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10,marginBottom:14}}><button type="button" onClick={()=>{setShipmentView('shipped');setSelectedBuyerKey('');setSelectedBuyerLookup('');setBuyerCandidateSnapshot([]);setSelectedProduct(null);setProductSearch('');setProductBuyerSearch('');setOrders([])}} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='shipped'?'#059669':'var(--border)'}`,background:shipmentView==='shipped'?'#ecfdf5':'var(--surface)',fontWeight:900,color:shipmentView==='shipped'?'#047857':'var(--text-secondary)'}}><PackageCheck size={16}/> 已出貨查詢</button><button type="button" onClick={()=>{setShipmentView('pending');setArrivalView('all');setShowArchived(false);setSelectedBuyerKey('');setSelectedBuyerLookup('');setBuyerCandidateSnapshot([]);setSelectedProduct(null);setProductSearch('');setProductBuyerSearch('');setOrders([])}} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='pending'?'#d97706':'var(--border)'}`,background:shipmentView==='pending'?'#fff7ed':'var(--surface)',fontWeight:900,color:shipmentView==='pending'?'#b45309':'var(--text-secondary)'}}><Truck size={16}/> 待出貨訂單</button></div>
    {shipmentView==='shipped'&&<div className="no-print" style={{display:'flex',justifyContent:'flex-end',gap:10,alignItems:'center',marginBottom:14}}><button className={`btn btn-sm ${showArchived?'btn-primary':'btn-ghost'}`} onClick={()=>{setShowArchived(v=>!v);setSelectedBuyerKey('');setSelectedBuyerLookup('');setBuyerCandidateSnapshot([]);setSelectedProduct(null);setProductBuyerSearch('');setOrders([])}}>{showArchived?<><ArchiveRestore size={13}/>隱藏封存</>:<><Archive size={13}/>顯示封存</>}</button></div>}
    <div className="no-print" style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:14}}><button type="button" style={modeCardStyle(mode==='buyer')} onClick={()=>{setMode('buyer');setSelectedProduct(null)}}><div style={{fontSize:16,fontWeight:900}}>👥 依買家查詢</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>輸入後才送 SQL 搜尋</div></button><button type="button" style={modeCardStyle(mode==='product')} onClick={()=>{setMode('product');setSelectedBuyerKey('');setSelectedBuyerLookup('');setBuyerCandidateSnapshot([])}}><div style={{fontSize:16,fontWeight:900}}>📦 依商品查詢</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>只顯示此狀態下有數量的商品</div></button></div>
    ...
    {mode==='buyer'&&<div className="card no-print" style={{marginBottom:16}}><div className="card-header" style={{fontWeight:800}}><UserSearch size={16}/>搜尋{statusLabel}買家</div><div className="card-body"><div className="search-input-wrap"><Search size={14}/><input autoFocus value={buyerSearch} onChange={e=>{setBuyerSearch(e.target.value);setSelectedBuyerKey('');setSelectedBuyerLookup('');setBuyerCandidateSnapshot([])}} placeholder="姓名／手機／末兩碼／Line／FB" style={{padding:'10px 10px 10px 34px',width:'100%'}}/></div>{!buyerSearch.trim()&&<div style={{marginTop:10,fontSize:12,color:'var(--text-muted)'}}>請輸入搜尋文字；空白時不下載任何訂單資料。</div>}{buyerSearch.trim()&&!loading&&<div style={{marginTop:12,display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8}}>{visibleBuyerCandidates.slice(0,30).map(c=><button type="button" key={c.key} onClick={()=>{if(!selectedBuyerLookup)setBuyerCandidateSnapshot(buyerCandidates.slice(0,30));setSelectedBuyerKey(c.key);setSelectedBuyerLookup(c.name||buyerSearch.trim())}} style={{textAlign:'left',padding:'11px 12px',borderRadius:10,border:`2px solid ${selectedBuyerKey===c.key?'#7c3aed':'var(--border)'}`,background:selectedBuyerKey===c.key?'#f5f3ff':'var(--surface-2)'}}><strong>{c.name}</strong>{c.phone_last2&&<span className="badge badge-violet" style={{marginLeft:6}}>末碼 {c.phone_last2}</span>}<div style={{fontSize:11,color:'var(--text-secondary)',marginTop:4}}>{[c.phone,c.line_nick&&`Line ${c.line_nick}`,c.fb_name&&`FB ${c.fb_name}`].filter(Boolean).join(' ・ ')||'無其他辨識資料'}</div></button>)}</div>}</div></div>}
