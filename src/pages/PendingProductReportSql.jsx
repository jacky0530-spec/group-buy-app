...
export default function PendingProductReportSql(){
  ...
  return <div className="animate-fade">
    ...
    <div className="no-print" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10,marginBottom:14}}><button type="button" onClick={()=>{setShipmentView('shipped');setSelectedBuyerKey('');setSelectedBuyerLookup('');setSelectedProduct(null);setProductSearch('');setProductBuyerSearch('');setOrders([])}} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='shipped'?'#059669':'var(--border)'}`,background:shipmentView==='shipped'?'#ecfdf5':'var(--surface)',fontWeight:900,color:shipmentView==='shipped'?'#047857':'var(--text-secondary)'}}><PackageCheck size={16}/> 已出貨查詢</button><button type="button" onClick={()=>{setShipmentView('pending');setArrivalView('all');setShowArchived(false);setSelectedBuyerKey('');setSelectedBuyerLookup('');setSelectedProduct(null);setProductSearch('');setProductBuyerSearch('');setOrders([])}} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='pending'?'#d97706':'var(--border)'}`,background:shipmentView==='pending'?'#fff7ed':'var(--surface)',fontWeight:900,color:shipmentView==='pending'?'#b45309':'var(--text-secondary)'}}><Truck size={16}/> 待出貨訂單</button></div>
    {shipmentView==='shipped'&&<div className="no-print" style={{display:'flex',justifyContent:'flex-end',gap:10,alignItems:'center',marginBottom:14}}><button className={`btn btn-sm ${showArchived?'btn-primary':'btn-ghost'}`} onClick={()=>{setShowArchived(v=>!v);setSelectedBuyerKey('');setSelectedBuyerLookup('');setSelectedProduct(null);setProductBuyerSearch('');setOrders([])}}>{showArchived?<><ArchiveRestore size={13}/>隱藏封存</>:<><Archive size={13}/>顯示封存</>}</button></div>}
    <div className="no-print" style={{display:'flex',gap:10,flexWrap:'wrap',marginBottom:14}}><button type="button" style={modeCardStyle(mode==='buyer')} onClick={()=>{setMode('buyer');setSelectedProduct(null)}}><div style={{fontSize:16,fontWeight:900}}>👥 依買家查詢</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>輸入後才送 SQL 搜尋</div></button><button type="button" style={modeCardStyle(mode==='product')} onClick={()=>{setMode('product');setSelectedBuyerKey('');setSelectedBuyerLookup('')}}><div style={{fontSize:16,fontWeight:900}}>📦 依商品查詢</div><div style={{fontSize:12,color:'var(--text-secondary)',marginTop:4}}>只顯示此狀態下有數量的商品</div></button></div>
    ...
    {mode==='buyer'&&<div className="card no-print" style={{marginBottom:16}}><div className="card-header" style={{fontWeight:800}}><UserSearch size={16}/>搜尋{statusLabel}買家</div><div className="card-body"><div className="search-input-wrap"><Search size={14}/><input autoFocus value={buyerSearch} onChange={e=>{setBuyerSearch(e.target.value);setSelectedBuyerKey('');setSelectedBuyerLookup('')}} placeholder="姓名／手機／末兩碼／Line／FB" style={{padding:'10px 10px 10px 34px',width:'100%'}}/></div>{!buyerSearch.trim()&&<div style={{marginTop:10,fontSize:12,color:'var(--text-muted)'}}>請輸入搜尋文字；空白時不下載任何訂單資料。</div>}{buyerSearch.trim()&&!loading&&<div style={{marginTop:12,display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(210px,1fr))',gap:8}}>{buyerCandidates.slice(0,30).map(c=><button type="button" key={c.key} onClick={()=>{setSelectedBuyerKey(c.key);setSelectedBuyerLookup(c.name||buyerSearch.trim())}} style={{textAlign:'left',padding:'11px 12px',borderRadius:10,border:`2px solid ${selectedBuyerKey===c.key?'#7c3aed':'var(--border)'}`,background:selectedBuyerKey===c.key?'#f5f3ff':'var(--surface-2)'}}><strong>{c.name}</strong>{c.phone_last2&&<span className="badge badge-violet" style={{marginLeft:6}}>末碼 {c.phone_last2}</span>}<div style={{fontSize:11,color:'var(--text-secondary)',marginTop:4}}>{[c.phone,c.line_nick&&`Line ${c.line_nick}`,c.fb_name&&`FB ${c.fb_name}`].filter(Boolean).join(' ・ ')||'無其他辨識資料'}</div></button>)}</div>}</div></div>}
