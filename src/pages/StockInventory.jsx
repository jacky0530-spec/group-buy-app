...
import { ArchiveRestore, Boxes, PackageCheck, Plus, RefreshCw, Search, Warehouse } from 'lucide-react'
...
export default function StockInventory(){
  ...
  const load=useCallback(async()=>{
    try{
      const [s,e,p]=await Promise.all([InventoryAPI.listStock(),InventoryAPI.listExtras(),ProductsAPI.list()])
      setStock(s);setExtras(e);setProducts(p)
  ...
  return <div className="animate-fade">
    <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',gap:12,flexWrap:'wrap',marginBottom:18}}><div><h2 style={{fontSize:22,fontWeight:900}}>現貨庫存</h2><p style={{fontSize:13,color:'var(--text-secondary)',marginTop:3}}>額外叫貨入庫後成為可售現貨；現貨開單會直接扣庫存，不再列入供應商應訂數量。</p></div><button className="btn btn-ghost" onClick={load}><RefreshCw size={14}/>重新整理</button></div>
