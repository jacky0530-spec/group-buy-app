...
export default function Orders() {
  ...
  const [search,setSearch] = useState(''); const [filterStatus,setFilterStatus] = useState('all'); const [filterPayment,setFilterPayment] = useState('all'); const [filterProduct,setFilterProduct] = useState(''); const [filterDateFrom,setFilterDateFrom] = useState(''); const [filterDateTo,setFilterDateTo] = useState(''); const [showArchived,setShowArchived] = useState(false); const [selected,setSelected] = useState([])
  ...
  const filtered = visibleOrders.filter(o => { const q = search.toLowerCase().trim(); const ms = (o.customer_name || '').toLowerCase().includes(q) || orderPhoneLast2(o).toLowerCase().includes(q) || String(o.customer_phone || customerMap[o.customer_id]?.phone || '').includes(search.trim()) || (o.items || []).some(i => (i.product_name || i.name || '').toLowerCase().includes(q)); const productKeyword = filterProduct.toLowerCase().trim(); const productMatch = !productKeyword || (o.items || []).some(i => (i.product_name || i.name || '').toLowerCase().includes(productKeyword)); const orderDate = o.order_date ? new Date(o.order_date) : null; const orderDay = orderDate && !Number.isNaN(orderDate.getTime()) ? `${orderDate.getFullYear()}-${String(orderDate.getMonth()+1).padStart(2,'0')}-${String(orderDate.getDate()).padStart(2,'0')}` : ''; const dateMatch = (!filterDateFrom || orderDay >= filterDateFrom) && (!filterDateTo || orderDay <= filterDateTo); return ms && productMatch && dateMatch && (filterStatus === 'all' || o.status === filterStatus) && (filterPayment === 'all' || o.payment_status === filterPayment) })
  ...
  const hasOrderFilters = filterProduct.trim() || filterDateFrom || filterDateTo
  ...
  async function bulkDeleteSelected() {
    ...
    const productText = filterProduct.trim() ? `商品關鍵字「${filterProduct.trim()}」` : '全部商品'
