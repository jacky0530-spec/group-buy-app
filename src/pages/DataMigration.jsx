...
import { Database, RefreshCw, Play, CheckCircle2, AlertTriangle } from 'lucide-react'
...
const TABLES = [
  ...
  { key:'stock_purchase_extras', label:'額外叫貨', collection:'stock_purchase_extras', final:true },
  { key:'expenses', label:'其他費用', collection:'expenses', final:true },
]
...
async function postMigration(user,action,rows=[],final=false){
  const token = await user.getIdToken(true)
  const response = await fetch(final ? '/api/neon-migrate-final' : '/api/neon-migrate',{
    method:'POST',
...
export default function DataMigration(){
  ...
  const inspect = useCallback(async()=>{
    ...
    try{
      ...
      const [coreResult,finalResult] = await Promise.all([
        postMigration(user,'counts',[],false),
        postMigration(user,'counts',[],true),
      ])
      setTarget({...(coreResult.counts||{}),...(finalResult.counts||{})})
    }catch(err){
  ...
  async function migrate(){
    ...
    if(!window.confirm('開始將目前 Firestore 的完整業務資料複製到 Neon？\n\n包含帳號、客戶、商品、訂單、小幫手、現貨、供應商付款、額外叫貨與其他費用。此操作不會刪除 Firebase 原資料，可以重複執行。')) return
    ...
    try{
      for(const table of TABLES){
        ...
        for(const batch of chunks(rows,batchSize)){
          await postMigration(user,table.key,batch,table.final===true)
          done += batch.length
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap',marginBottom:16}}>
      <div><h2 style={{margin:0,display:'flex',gap:8,alignItems:'center'}}><Database size={22}/>資料庫搬移</h2><div style={{fontSize:13,color:'var(--text-muted)',marginTop:5}}>Firebase Authentication 保留；Firestore 業務資料完整複製到 Neon PostgreSQL。</div></div>
      <div style={{display:'flex',gap:8}}><button className="btn btn-ghost" disabled={loading||running} onClick={inspect}><RefreshCw size={14}/>{loading?'讀取中...':'重新分析'}</button><button className="btn btn-primary" disabled={loading||running||!source.orders} onClick={migrate}><Play size={14}/>{running?'搬移中...':'開始完整搬移'}</button></div>
    </div>
    ...
    <div className="card" style={{padding:18}}>
      <strong>搬移原則</strong>
      <div style={{fontSize:13,lineHeight:1.8,color:'var(--text-secondary)',marginTop:8}}>原 Firestore 文件 ID 會保存成 legacy_id／firebase_uid 或透過商品＋規格唯一鍵對應，用來避免重複匯入並保留資料關聯。再次執行會更新既有資料，不會重複建立同一筆資料。Firebase 原資料在完成核對與正式切換前都不會刪除。</div>
    </div>
  </div>
}
