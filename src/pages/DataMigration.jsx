...
export default function DataMigration(){
  ...
  const inspect = useCallback(async()=>{
    ...
    try{
      ...
      setSource(next)
      try { const result=await postMigration(user,'counts',[]);setTarget(result.counts) }
      catch(err){ setTarget(null); throw err }
    }catch(err){ setError(err.message);toast('資料庫檢查失敗：'+err.message,'error') }
  ...
  async function migrate(){
    ...
    if(!window.confirm('開始將目前 Firestore 的帳號、客戶、商品、訂單複製到 Neon？\n\n此操作不會刪除 Firebase 原資料，可以重複執行。')) return
    setRunning(true);setError('');setProgress({})
    try{
      for(const table of TABLES){
        const rows = source[table.key] || []
        const batchSize = table.key==='orders' ? 100 : 200
        let done=0
  ...
  return <div>
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap',marginBottom:16}}>
      ...
      <div style={{display:'flex',gap:8}}><button className="btn btn-ghost" disabled={loading||running} onClick={inspect}><RefreshCw size={14}/>{loading?'讀取中...':'重新分析'}</button><button className="btn btn-primary" disabled={loading||running||!source.orders} onClick={migrate}><Play size={14}/>{running?'搬移中...':'開始搬移核心資料'}</button></div>
    </div>
    ...
    <div className="card" style={{padding:18}}>
      <strong>搬移原則</strong>
      <div style={{fontSize:13,lineHeight:1.8,color:'var(--text-secondary)',marginTop:8}}>原 Firestore 文件 ID 會保存在 Neon 的 legacy_id／firebase_uid，用來避免重複匯入並保留資料關聯。再次執行會更新既有資料，不會把同一筆客戶、商品或訂單重複建立。訂單明細會依訂單重新同步，因此可以用來反覆測試核對。</div>
    </div>
  </div>
}
