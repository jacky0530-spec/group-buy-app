...
export default function HelperPortalV3() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      setEntries(await HelperAPI.myEntries(user.uid,20))
    } catch (err) {
      toast('最近登記載入失敗：' + err.message, 'error')
    } finally {
      setLoading(false)
    }
  }, [user, toast])
  ...
  useEffect(() => {
    const q = (mode === 'customer' ? customerSearch : batchCustomerSearch).trim()
    if (!q) { setCustomers([]); setCustomerSearching(false); return undefined }
    let active = true
    setCustomerSearching(true)
    const timer = window.setTimeout(async () => {
      try {
        const rows = await HelperAPI.searchCustomers(q,mode === 'customer' ? 20 : 30)
        if (active) setCustomers(rows)
      } catch (err) {
        if (active) toast('客戶搜尋失敗：' + err.message,'error')
      } finally {
        if (active) setCustomerSearching(false)
      }
    },220)
    return () => { active=false; window.clearTimeout(timer) }
  }, [mode, customerSearch, batchCustomerSearch, toast])

  useEffect(() => {
    const opened = mode === 'customer' ? productOpen : batchProductOpen
    const q = mode === 'customer' ? productSearch : batchProductSearch
    if (!opened) { setProductSearching(false); return undefined }
    let active = true
    setProductSearching(true)
    const timer = window.setTimeout(async () => {
      try {
        const rows = await HelperAPI.searchCatalog(q,30)
        if (active) setProducts(rows)
      } catch (err) {
        if (active) toast('商品搜尋失敗：' + err.message,'error')
      } finally {
  ...
  return <div style={{ minHeight:'100vh', background:'#f8fafc' }}>
    ...
    <main style={{ maxWidth:980, margin:'0 auto', padding:16 }}>
      ...
      {mode==='customer'?<>
        <div className="card" style={{marginBottom:14}}><div className="card-header"><strong>① 選擇客戶</strong></div><div className="card-body"><div className="search-input-wrap"><Search size={16}/><input value={customerSearch} onChange={e=>setCustomerSearch(e.target.value)} placeholder="姓名／手機末兩碼／Line／FB" style={{paddingLeft:36,height:48,fontSize:16}}/></div>{customerSearch&&<div style={{marginTop:8,border:'1px solid var(--border)',borderRadius:10,overflow:'hidden'}}>{customerSearching?<div style={{padding:14,color:'var(--text-muted)'}}>SQL 搜尋中...</div>:custs.length?custs.map(c=><button key={c.id} onClick={()=>{setCustomer(c);setCustomerSearch('')}} style={{display:'block',width:'100%',textAlign:'left',padding:'10px 12px',border:0,borderBottom:'1px solid var(--border)',background:'#fff'}}><strong>{c.name}</strong>{customerIdentityLabel(c)&&<div style={{fontSize:11,color:'var(--text-muted)'}}>{customerIdentityLabel(c)}</div>}{customerNoteText(c)&&<div style={{fontSize:11,color:'#b45309',fontWeight:800,marginTop:2}}>備註：{customerNoteText(c)}</div>}{!customerIdentityLabel(c)&&!customerNoteText(c)&&<div style={{fontSize:11,color:'var(--text-muted)'}}>無其他辨識資料</div>}</button>):<div style={{padding:14,color:'var(--text-muted)'}}>找不到符合客戶</div>}</div>}{customer&&<div style={{marginTop:10,padding:12,borderRadius:10,background:'var(--indigo-light)',display:'flex',justifyContent:'space-between',gap:10,alignItems:'flex-start'}}><div><strong>{customer.name}（末碼 {getCustomerPhoneLast2(customer)||'—'}）</strong>{customerIdentityLabel(customer)&&<div style={{fontSize:11,color:'var(--text-secondary)',marginTop:3}}>{customerIdentityLabel(customer)}</div>}{customerNoteText(customer)&&<div style={{fontSize:11,color:'#b45309',fontWeight:800,marginTop:3}}>備註：{customerNoteText(customer)}</div>}{!customerIdentityLabel(customer)&&!customerNoteText(customer)&&<div style={{fontSize:11,color:'var(--text-secondary)',marginTop:3}}>無其他辨識資料</div>}</div><button onClick={()=>setCustomer(null)} style={{border:0,background:'transparent'}}><X size={16}/></button></div>}</div></div>
