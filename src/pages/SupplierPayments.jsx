...
export default function SupplierPayments(){
  ...
  const[selectedSupplier,setSelectedSupplier]=useState(''),[search,setSearch]=useState(''),[productSearch,setProductSearch]=useState(''),[selected,setSelected]=useState([])
  ...
  const visibleSupplierLines=useMemo(()=>{
    ...
    return supplierLines.filter(l=>!q||String(l.product_name||'').toLowerCase().includes(q)||String(l.spec||'').toLowerCase().includes(q))
  },[supplierLines,productSearch])
  const readyLines=useMemo(()=>supplierLines.filter(l=>l.isEligible&&!l.needsCost),[supplierLines])
  const visibleReadyLines=useMemo(()=>visibleSupplierLines.filter(l=>l.isEligible&&!l.needsCost),[visibleSupplierLines])
  ...
  async function chooseSupplier(name){
    setSelectedSupplier(name);setSelected([]);setPaymentAmount('');setNote('');setProductSearch('');setSupplierLines([]);setCostEdits({})
    await loadSupplier(name,{autoSelect:false})
  }
  function applySelection(keys){
    const clean=[...new Set(keys)].filter(key=>readyLines.some(l=>l.key===key))
    setSelected(clean)
    const total=readyLines.filter(l=>clean.includes(l.key)).reduce((sum,l)=>sum+Number(l.outstanding||0),0)
    setPaymentAmount(total?String(Math.round(total)):'')
  }
  function toggle(key){applySelection(selected.includes(key)?selected.filter(x=>x!==key):[...selected,key])}
  function selectAll(){applySelection([...selected,...visibleReadyLines.map(l=>l.key)])}
  ...
  return <div className="animate-fade">
    ...
    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(280px,1fr))',gap:14,alignItems:'start'}}>
      ...
      <div>{!selectedSupplier?<div className="card"><div className="empty-state" style={{padding:50}}>請先選擇左側供應商</div></div>:<>
        <div className="card" style={{marginBottom:14}}><div className="card-header" style={{display:'flex',justifyContent:'space-between',gap:10,flexWrap:'wrap',alignItems:'center'}}><strong>{selectedSupplier}｜待付款／成本確認明細</strong><div style={{display:'flex',gap:6}}><button className="btn btn-sm btn-ghost" disabled={detailsLoading||!visibleReadyLines.length} onClick={selectAll}>{productSearch.trim()?'搜尋結果全選':'全部勾選'}</button><button className="btn btn-sm btn-ghost" disabled={detailsLoading||!selected.length} onClick={clearAll}>清除勾選</button></div><div style={{width:'100%',display:'flex',gap:8,alignItems:'center',flexWrap:'wrap'}}><div className="search-input-wrap" style={{flex:'1 1 280px',minWidth:0}}><Search size={19}/><input value={productSearch} onChange={e=>setProductSearch(e.target.value)} placeholder="搜尋商品名稱／規格..." style={{...INPUT_STYLE,paddingLeft:44,width:'100%'}}/></div><div style={{fontSize:12,color:'var(--text-muted)',whiteSpace:'nowrap'}}>顯示 {visibleSupplierLines.length} / {supplierLines.length} 筆｜已勾 {selected.length} 筆</div></div></div><div className="table-container"><table><thead><tr><th></th><th>客戶 / 商品</th><th>付款條件</th><th>到貨</th><th>單位成本 / 總成本</th><th>已付</th><th>待付</th></tr></thead><tbody>{detailsLoading&&<tr><td colSpan={7} style={{textAlign:'center',padding:26}}>明細讀取中...</td></tr>}{!detailsLoading&&visibleSupplierLines.map(l=><tr key={l.key} style={{opacity:l.needsCost?1:(l.isEligible?1:.55),background:l.needsCost?'#fff7ed':undefined}}><td><input type="checkbox" disabled={!l.isEligible||l.needsCost} checked={selected.includes(l.key)} onChange={()=>toggle(l.key)}/></td><td><div style={{fontWeight:800}}>{l.product_name} ×{l.qty} {l.isStockPurchase&&<span className="badge badge-violet">現貨進貨</span>} {l.needsCost&&<span className="badge badge-rose">成本待確認</span>}</div><div style={{fontSize:11,color:'var(--indigo)',fontWeight:700}}>{l.spec}</div><div style={{fontSize:11,color:'var(--text-muted)'}}>{l.customer_name}</div></td><td><span className={`badge ${l.term==='order'?'badge-indigo':l.term==='arrival'?'badge-amber':'badge-gray'}`}>{termLabel(l.term)}</span></td><td>{l.arrived?<span className="badge badge-emerald">已到貨</span>:<span className="badge badge-rose">未到貨</span>}</td><td style={{minWidth:205}}>{l.isStockPurchase?<><div style={{fontWeight:800}}>{money(l.unitCost)} / 件</div><div style={{fontSize:11,color:'var(--text-muted)'}}>總成本 {money(l.cost)}</div></>:<><div style={{display:'flex',gap:6,alignItems:'center'}}><input type="number" min="0.01" step="0.01" value={costEdits[l.key]??''} onChange={e=>setCostEdits(p=>({...p,[l.key]:e.target.value}))} style={{...INPUT_STYLE,width:112}}/><button className="btn btn-sm btn-ghost" disabled={costSaving===l.key} onClick={()=>saveCost(l)}>{costSaving===l.key?'儲存中':'儲存'}</button></div><div style={{fontSize:11,marginTop:4,color:l.needsCost?'#b91c1c':'var(--text-muted)'}}>{l.needsCost?(Number(l.suggestedUnitCost)>0?`商品目前成本建議 ${money(l.suggestedUnitCost)}；請確認後儲存`:'商品成本也是 0，請依帳單輸入'):`目前總成本 ${money(l.cost)}；帳單不同可直接修正`}</div></>}</td><td>{money(l.paid)}</td><td style={{fontWeight:900,color:l.needsCost?'#b91c1c':l.isEligible?'var(--rose)':'var(--text-muted)'}}>{l.needsCost?'先確認成本':money(l.outstanding)}</td></tr>)}{!detailsLoading&&!supplierLines.length&&<tr><td colSpan={7} style={{textAlign:'center',padding:24,color:'var(--text-muted)'}}>此供應商目前沒有待付款或成本待確認明細</td></tr>}{!detailsLoading&&supplierLines.length>0&&!visibleSupplierLines.length&&<tr><td colSpan={7} style={{textAlign:'center',padding:24,color:'var(--text-muted)'}}>找不到符合「{productSearch}」的商品或規格</td></tr>}</tbody></table></div></div>
