...
export default function SupplierPayments(){
  ...
  const[orders,setOrders]=useState([]),[payments,setPayments]=useState([]),[stockPayables,setStockPayables]=useState([]),[loading,setLoading]=useState(true)
  ...
  const load=useCallback(async()=>{setLoading(true);try{const[o,p,s]=await Promise.all([OrdersAPI.list(),SupplierPaymentsAPI.list(),neonPaymentsRuntime('list_stock_payables')]);setOrders(o);setPayments(p);setStockPayables(Array.isArray(s?.rows)?s.rows:[])}catch(e){toast('供應商付款資料載入失敗：'+e.message,'error')}finally{setLoading(false)}},[toast])
  useEffect(()=>{load()},[load])

  const lines=useMemo(()=>{
    const orderLines=orders.filter(o=>o.status!=='cancelled'&&!o.archived&&!o.is_virtual).flatMap(o=>(o.items||[]).map((item,index)=>({
      key:`${o.id}:${index}`,order_id:o.id,item_index:index,extra_id:'',customer_name:o.customer_name||'',order_date:o.order_date,
      supplier:item.supplier||'未指定供應商',product_name:item.product_name||item.name||'未命名商品',spec:specText(item),qty:Number(item.qty||0),
      cost:itemCost(item),paid:itemPaid(item),outstanding:itemOutstanding(item),term:item.supplier_payment_term||'manual',arrived:itemArrived(item),isEligible:eligible(item),isStockPurchase:false,item
    }))).filter(x=>x.outstanding>0.001)
    const stockLines=(stockPayables||[]).map(x=>{
      const cost=Number(x.unit_cost||0)*Number(x.ordered_qty||0),paid=Math.max(0,Number(x.supplier_paid_amount||0))
      return {key:`stock:${x.extra_id}`,order_id:'',item_index:0,extra_id:x.extra_id,customer_name:'現貨進貨',order_date:x.created_at,supplier:x.supplier||'未指定供應商',product_name:x.product_name||'未命名商品',spec:x.spec_label||'一般規格',qty:Number(x.ordered_qty||0),cost,paid,outstanding:Math.max(0,cost-paid),term:'manual',arrived:Number(x.received_qty||0)>=Number(x.ordered_qty||0),isEligible:true,isStockPurchase:true,item:null}
    }).filter(x=>x.outstanding>0.001)
    return [...orderLines,...stockLines]
  },[orders,stockPayables])
  ...
  return <div className="animate-fade">
    <div style={{display:'flex',justifyContent:'space-between',gap:12,alignItems:'center',flexWrap:'wrap',marginBottom:18}}><div><h2 style={{fontSize:22,fontWeight:800}}>供應商付款中心</h2><p style={{fontSize:13,color:'var(--text-secondary)',marginTop:3}}>訂貨即付款與到貨後付款會在到貨時由 Neon 自動建立付款紀錄；只有「手動付款」需要在此勾選並確認。現貨額外叫貨也會併入同一供應商帳務。</p></div><button className="btn btn-ghost" onClick={load}><RefreshCw size={14}/>重新整理</button></div>

    <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:12,marginBottom:18}}>
      <div className="stat-card" style={{background:'linear-gradient(135deg,#f59e0b,#d97706)'}}><div style={{fontSize:11,fontWeight:800}}>目前可手動付款</div><div style={{fontSize:22,fontWeight:900,marginTop:5}}>{money(totalReady)}</div><WalletCards size={28} style={{position:'absolute',right:14,top:18,opacity:.25}}/></div>
