...
export default function IncomingBatches(){
  ...
  const selectVisible=()=>setSelectedKeys([...new Set([...selectedKeys,...visibleCandidates.map(keyOf)])])
  ...
  const createBatch=async()=>{
    ...
    try{
      ...
      await loadHome()
      if(data?.result?.id) openBatch(data.result.id)
  ...
  const openBatch=id=>{
    ...
    setCompletion(batch.status==='completed'?{affected:[],completed:true}:null)
  ...
  const loadBatchPaymentLines=async(batch,affected)=>{
    try{
      ...
      return rows
    }catch(e){toast('本批待付款明細載入失敗：'+e.message,'error');return[]}
  }
  ...
  const completeBatch=async()=>{
    ...
    try{
      ...
      setBatches(updated?.result||[])
      const finished=(updated?.result||[]).find(b=>b.id===activeBatch.id)||activeBatch
  ...
  const payBatch=async()=>{
    ...
    setPaying(true)
    try{
      const result=await SupplierPaymentsAPI.createPayment({supplier:activeBatch.supplier,payment_date:paymentDate,amount,note:`即將到貨批次 ${activeBatch.id}${paymentNote?`｜${paymentNote}`:''}`,lines:paymentLines})
      toast(`✅ 本批已建立付款 ${money(result.amount)}，分配 ${result.allocation_count} 筆`)
      setPaymentLines([]);setPaymentAmount('');setPaymentNote('')
  ...
  return <div className="animate-fade">
    ...
    <div className="no-print" style={{display:'grid',gridTemplateColumns:'minmax(260px,.8fr) minmax(360px,1.4fr)',gap:14,alignItems:'start'}}>
