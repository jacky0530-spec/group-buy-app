...
import { ClipboardList, LogOut, Pencil, RefreshCw, Save, ShoppingCart, X } from 'lucide-react'
  ...
  return <div>
    ...
    {!loading&&filtered.map(order=>{
      ...
      return <div className="card" key={order.id} style={{marginBottom:12,border:order.is_virtual?'1.5px solid #fb7185':undefined,background:order.is_virtual?'#fff7f8':undefined}}>
        <div className="card-header" style={{display:'flex',justifyContent:'space-between',gap:10,alignItems:'center',flexWrap:'wrap'}}><div><strong>{order.customer_name}</strong> <span style={{fontSize:11,color:'var(--text-muted)'}}>末碼 {order.customer_phone_last2||'—'}</span><div style={{fontSize:11,color:'var(--text-muted)',marginTop:2}}>訂購：{dateText(order.order_date||order.created_at)}</div></div><div style={{display:'flex',gap:7,alignItems:'center'}}><span className={`badge ${order.is_virtual?'badge-rose':'badge-emerald'}`}>{order.is_virtual?'虛擬':'正式'}</span>{locked?<span className="badge badge-amber">已到貨，已鎖定</span>:!isEditing?<button className="btn btn-sm btn-ghost" onClick={()=>begin(order)}><Pencil size={12}/>修改</button>:null}</div></div>
        <div className="card-body">
          {!isEditing?(order.items||[]).map((item,i)=><div key={i} style={{padding:'8px 0',borderBottom:i<(order.items||[]).length-1?'1px dashed var(--border)':'none'}}><strong>{item.product_name||item.name}</strong> × <strong>{item.qty}</strong><div style={{fontSize:12,color:'var(--indigo)',fontWeight:800,marginTop:3}}>{[item.spec?.package,item.spec?.flavor,item.spec?.color,item.spec?.size].filter(Boolean).join('／')||'無規格'}</div>{item.note&&<div style={{fontSize:11,color:'var(--text-muted)'}}>備註：{item.note}</div>}</div>):draft&&<>
...
export default function HelperPortalV4(){
  ...
  return <div style={{minHeight:'100vh',background:'#f8fafc'}}>
    ...
    <div style={{position:'sticky',top:61,zIndex:35,background:'#f8fafc',padding:'10px 16px 8px',borderBottom:'1px solid #e5e7eb'}}><div style={{maxWidth:980,margin:'0 auto',display:'grid',gridTemplateColumns:'1fr 1fr',gap:10}}><button onClick={()=>setTab('new')} style={{height:50,borderRadius:12,border:`2px solid ${tab==='new'?'var(--indigo)':'var(--border)'}`,background:tab==='new'?'var(--indigo-light)':'#fff',fontWeight:900,color:tab==='new'?'var(--indigo)':'var(--text-primary)'}}><ShoppingCart size={15}/> 新增訂單</button><button onClick={()=>setTab('pending')} style={{height:50,borderRadius:12,border:`2px solid ${tab==='pending'?'#059669':'var(--border)'}`,background:tab==='pending'?'#ecfdf5':'#fff',fontWeight:900,color:tab==='pending'?'#047857':'var(--text-primary)'}}><ClipboardList size={15}/> 我的未出貨</button></div></div>
    {tab==='new'?<div className="helper-v4-new"><HelperPortalV3/></div>:<main style={{maxWidth:980,margin:'0 auto',padding:16}}><MyPendingOrders/></main>}
  </div>
}
