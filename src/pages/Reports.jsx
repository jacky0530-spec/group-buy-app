...
import { TrendingUp, DollarSign, Package, Search, Printer, Download, WalletCards, ReceiptText, Building2, Truck } from 'lucide-react'
...
function periodBounds(mode, month, start, end) {
  if (mode === 'month' && month) { const [y,m]=month.split('-').map(Number); return [new Date(y,m-1,1).toISOString(),new Date(y,m,0,23,59,59,999).toISOString()] }
  if (mode === 'range' && start && end) return [new Date(`${start}T00:00:00`).toISOString(),new Date(`${end}T23:59:59`).toISOString()]
  return [null,null]
}
function expenseInPeriod(row, mode, month, start, end) {
  if (mode === 'all') return true
  if (mode === 'month') return row.month === month
  if (mode === 'range') {
    if (!start || !end) return false
    const first = `${row.month}-01`
    const [y,m] = row.month.split('-').map(Number)
    const last = `${row.month}-${String(new Date(y,m,0).getDate()).padStart(2,'0')}`
...
function exportCsv(orders, expenses, filename) {
  const rows=[['類別','日期/月','客戶/廠商','商品/費用類型','規格/備註','數量','收入','成本/費用']]
  orders.forEach(order=>(order.items||[]).forEach(item=>rows.push(['訂單',dateText(order.order_date),order.customer_name,item.product_name||item.name,specText(item),item.qty,Number(item.sale_price??item.price??0)*Number(item.qty||0),Number(item.cost_price||0)*Number(item.qty||0)])))
  expenses.forEach(row=>rows.push(['其他費用',row.month,row.supplier,EXPENSE_TYPES.find(t=>t.id===row.type)?.label||row.type,row.note||'',1,'',expenseSignedAmount(row)]))
...
export default function Reports() {
  ...
  const [orders,setOrders]=useState([]), [products,setProducts]=useState([]), [expenses,setExpenses]=useState([])
  const [loading,setLoading]=useState(true), [error,setError]=useState('')
  const [filterMode,setFilterMode]=useState('month'), [inputMonth,setInputMonth]=useState(currentMonth), [inputStart,setInputStart]=useState(''), [inputEnd,setInputEnd]=useState('')
  const [activeTab,setActiveTab]=useState('summary'), [trackProd,setTrackProd]=useState(null), [prodSearch,setProdSearch]=useState('')
  ...
  const periodExpenses=expenses.filter(row=>expenseInPeriod(row,filterMode,inputMonth,inputStart,inputEnd))
  const expenseNet=periodExpenses.reduce((s,r)=>s+expenseSignedAmount(r),0)
  const expenseShipping=periodExpenses.filter(r=>r.type==='shipping').reduce((s,r)=>s+Number(r.amount||0),0)
  const expenseOther=periodExpenses.filter(r=>r.type==='other').reduce((s,r)=>s+Number(r.amount||0),0)
  const expenseDiscount=periodExpenses.filter(r=>r.type==='discount').reduce((s,r)=>s+Number(r.amount||0),0)
  ...
  const orderValue=validOrders.reduce((s,o)=>s+effectiveOrderAmount(o),0), shippedRevenue=shippedOrders.reduce((s,o)=>s+effectiveOrderAmount(o),0), shippedCost=shippedOrders.reduce((s,o)=>s+orderSnapshotCost(o,currentCostMap),0)
  const baseProfit=shippedRevenue-shippedCost, adjustedProfit=baseProfit-expenseNet
  const collectedAmount=validOrders.filter(o=>['paid','partial_refund','refunded'].includes(o.payment_status)).reduce((s,o)=>s+effectiveOrderAmount(o),0)
  const outstandingAmount=validOrders.filter(o=>o.payment_status==='unpaid').reduce((s,o)=>s+effectiveOrderAmount(o),0), refundAmount=validOrders.reduce((s,o)=>s+Number(o.refund_amount||0),0), cancelledAmount=cancelledOrders.reduce((s,o)=>s+Number(o.total_amount||0),0)
  const supplierPaidCost=validOrders.filter(o=>o.payable_status==='paid').reduce((s,o)=>s+orderSnapshotCost(o,currentCostMap),0), payableOutstanding=validOrders.filter(o=>o.payable_status!=='paid').reduce((s,o)=>s+orderSnapshotCost(o,currentCostMap),0)
  const netCashFlow=collectedAmount-supplierPaidCost-expenseNet
  ...
  const supplierRows=useMemo(()=>{const map={};validOrders.forEach(o=>(o.items||[]).forEach(i=>{const supplier=i.supplier||'未指定供應商';if(!map[supplier])map[supplier]={supplier,total:0,paid:0,outstanding:0,adjustments:0};const cost=Number(i.cost_price??currentCostMap[i.product_id||i.id]||0)*Number(i.qty||0);map[supplier].total+=cost;if(o.payable_status==='paid')map[supplier].paid+=cost;else map[supplier].outstanding+=cost}));periodExpenses.forEach(e=>{if(!map[e.supplier])map[e.supplier]={supplier:e.supplier,total:0,paid:0,outstanding:0,adjustments:0};map[e.supplier].adjustments+=expenseSignedAmount(e)});return Object.values(map).sort((a,b)=>(b.outstanding+b.adjustments)-(a.outstanding+a.adjustments))},[validOrders,currentCostMap,periodExpenses])
