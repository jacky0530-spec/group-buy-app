...
export default function ExcelOrderImport(){
  const toast=useToast();const[products,setProducts]=useState([]);const[customers,setCustomers]=useState([]);const[orders,setOrders]=useState([]);const[groups,setGroups]=useState([]);const[loading,setLoading]=useState(true);const[catalogReady,setCatalogReady]=useState(false);const[duplicateReady,setDuplicateReady]=useState(false);const[parsing,setParsing]=useState(false);const[saving,setSaving]=useState(false);const[importConfirm,setImportConfirm]=useState(null)
  ...
  function importOrders(){
    ...
    preview.forEach(g=>g.buyers.forEach(b=>{if(b.duplicate)return;const items=b.resolved.map(r=>snapshotOrderItem(g.product,{qty:r.qty,spec:r.spec,priceOption:r.priceOption}));rows.push({customer_id:b.customer.id,customer_name:b.customer.name,customer_phone:b.customer.phone||'',customer_phone_last2:getCustomerPhoneLast2(b.customer),items,total_amount:items.reduce((sum,item)=>sum+item.subtotal,0),note:`Excel匯入：${g.fileName}${b.marker?`；原表標記：${b.marker}`:''}`,is_virtual:Boolean(b.is_virtual),source:'excel_import',import_batch_id:batchId,import_source_file:g.fileName,import_source_sheet:g.sheetName,import_signature:b.sig,imported_at:new Date().toISOString()})}))
    if(!rows.length){toast('沒有新的可匯入訂單','warning');return}
    setImportConfirm({rows,count:rows.length,batchId})
  }
  async function executeImport(){
    const rows=importConfirm?.rows||[]
    if(!rows.length||saving)return
    setSaving(true)
    try{
      let created=0
      for(let i=0;i<rows.length;i+=250){
        const part=await OrdersAPI.batchCreate(rows.slice(i,i+250))
        created+=part.length
      }
      toast(`Excel 匯入完成：建立 ${created} 筆訂單 ✓`)
      setImportConfirm(null);setGroups([]);await load()
    }catch(err){
  ...
  return <div className="animate-fade">
    ...
    {importConfirm&&<Modal title="確認匯入 Excel 訂單" onClose={()=>!saving&&setImportConfirm(null)} width={520}>
      <div style={{fontSize:15,lineHeight:1.7}}>
        <div style={{background:'#eef2ff',border:'1px solid #c7d2fe',borderRadius:12,padding:'14px 16px',marginBottom:14}}>
          <div style={{fontSize:18,fontWeight:900,color:'#3730a3',marginBottom:4}}>確定建立 {importConfirm.count} 筆訂單？</div>
          <div style={{fontSize:13,color:'#475569'}}>相同客戶＋商品＋規格＋原訂數量的既有 Excel 訂單會自動略過。</div>
        </div>
        <div style={{fontSize:12,color:'var(--text-secondary)',marginBottom:16}}>此確認視窗由系統頁面直接顯示，不再使用瀏覽器原生 window.confirm，因此 iPad／Safari／平板瀏覽器不會因原生阻擋式對話框而卡住。按下建立後請停留在此頁，完成前按鈕會顯示「建立訂單中…」。</div>
        <div style={{display:'flex',gap:10,justifyContent:'flex-end',flexWrap:'wrap'}}>
          <button className="btn btn-ghost" disabled={saving} onClick={()=>setImportConfirm(null)}>取消</button>
          <button className="btn btn-primary" disabled={saving} onClick={executeImport} style={{minWidth:180,height:48,fontSize:15,fontWeight:900}}>{saving?'建立訂單中…':`確認建立 ${importConfirm.count} 筆訂單`}</button>
        </div>
      </div>
    </Modal>}
  </div>
