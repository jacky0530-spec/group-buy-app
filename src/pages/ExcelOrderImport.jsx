...
export default function ExcelOrderImport(){
  ...
  const load=useCallback(async()=>{setLoading(true);try{const[p,c,o]=await Promise.all([ProductsAPI.list(),CustomersAPI.list(),OrdersAPI.list()]);setProducts(p);setCustomers(c);setOrders(o)}catch(err){toast('載入失敗：'+err.message,'error')}finally{setLoading(false)}},[toast])
  useEffect(()=>{load()},[load])
