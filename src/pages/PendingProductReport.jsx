...
export default function PendingProductReport() {
  ...
  return <div className="animate-fade">
    ...
    <div className="no-print" style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(220px,1fr))',gap:10,marginBottom:14}}><button type="button" onClick={() => { setShipmentView('pending'); setArrivalView('all') }} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='pending'?'#d97706':'var(--border)'}`,background:shipmentView==='pending'?'#fff7ed':'var(--surface)',fontWeight:900,color:shipmentView==='pending'?'#b45309':'var(--text-secondary)',cursor:'pointer',fontFamily:'inherit'}}><Truck size={16} style={{verticalAlign:'middle',marginRight:7}}/>待出貨訂單</button><button type="button" onClick={() => setShipmentView('shipped')} style={{borderRadius:12,padding:'12px 16px',border:`2px solid ${shipmentView==='shipped'?'#059669':'var(--border)'}`,background:shipmentView==='shipped'?'#ecfdf5':'var(--surface)',fontWeight:900,color:shipmentView==='shipped'?'#047857':'var(--text-secondary)',cursor:'pointer',fontFamily:'inherit'}}><PackageCheck size={16} style={{verticalAlign:'middle',marginRight:7}}/>已出貨查詢</button></div>
