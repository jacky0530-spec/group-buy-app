...
import { neonOrderQuery } from '../lib/neonRuntime'
...
export default function Customers() {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const result = await neonOrderQuery('customer_directory',{ includeArchived:showArchived })
      if (seq !== loadSeqRef.current) return
      if (!Array.isArray(result?.rows)) throw new Error('客戶 SQL 回傳格式錯誤')
      setCustomers(result.rows.map(row => ({ ...row,order_count:Number(row.order_count || 0) })))
  ...
  const filtered = customers.filter(c => customerMatchesSearch(c,q)).sort((a,b) => {
    ...
    if (sortBy === 'orders_desc') {
      const diff = Number(b.order_count || 0) - Number(a.order_count || 0)
  ...
  return <div className="animate-fade">
    <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20,flexWrap:'wrap',gap:12 }}>
      <div>
        ...
        <p style={{ color:'var(--text-secondary)',fontSize:13,marginTop:2 }}>共 {customers.length} 位　姓名可重複；辨識末碼可自訂 2 碼、3 碼以上</p>
      </div>
    ...
    <div style={{ background:'var(--sky-light)',borderRadius:8,padding:'9px 12px',marginBottom:14,fontSize:12,color:'#0369a1' }}>
      📱 辨識末碼可重複，也可自行輸入 2～3 碼以上，例如「00」或「000」；完整電話填入後會自動帶出末兩碼。有效訂單數由資料庫直接統計；搜尋範圍包含全部客戶與備註。
    </div>

    <div style={{ marginBottom:14,display:'flex',gap:10,flexWrap:'wrap',alignItems:'center' }}>
      <div className="search-input-wrap" style={{ flex:'1 1 360px',maxWidth:520 }}>
        <Search size={14}/>
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="搜尋姓名、辨識末碼、完整電話、Line、FB、備註..." style={{ padding:'8px 8px 8px 32px',border:'1.5px solid var(--border)',borderRadius:8,fontSize:14,outline:'none',fontFamily:'inherit',background:'var(--surface)',width:'100%' }}/>
      </div>
        ...
        <table>
          ...
          <tbody>
            ...
            {displayed.map(c => {
              ...
              return <tr key={c.id} style={{ opacity:archived ? .55 : 1 }}>
                ...
                <td>{orderCount ? <span className="badge badge-indigo">{orderCount} 筆</span> : '0'}</td>
    ...
    {showModal && <Modal title={editId ? '編輯客戶資料' : '新增客戶'} onClose={() => setShowModal(false)}>
      <div style={{ background:'var(--sky-light)',borderRadius:8,padding:'9px 12px',marginBottom:14,fontSize:12,color:'#0369a1' }}>ℹ️ 同名客戶可以存在；請用辨識末碼、完整電話、Line、FB 或備註協助辨識。</div>
